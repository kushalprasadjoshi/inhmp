...
router = APIRouter(prefix="/auth", tags=["Authentication"])   # ← PREFIX IS /auth
...
def login(credentials: LoginRequest, db: Session = Depends(get_db)):
    ...
    access_token = service.create_access_token({"sub": str(UUID(bytes=user.id)), "role": user.role})
